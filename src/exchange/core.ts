...
import { updatePairDayData, updateTokenDayData, updateAliumDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  alium.totalLiquidityBNB = alium.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  alium.totalLiquidityBNB = alium.totalLiquidityBNB.plus(trackedLiquidityBNB);
  alium.totalLiquidityUSD = alium.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  alium.totalTransactions = alium.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let alium = AliumFactory.load(FACTORY_ADDRESS);
  alium.totalVolumeUSD = alium.totalVolumeUSD.plus(trackedAmountUSD);
  alium.totalVolumeBNB = alium.totalVolumeBNB.plus(trackedAmountBNB);
  alium.untrackedVolumeUSD = alium.untrackedVolumeUSD.plus(derivedAmountUSD);
  alium.totalTransactions = alium.totalTransactions.plus(ONE_BI);
  ...
  let aliumDayData = updateAliumDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  aliumDayData.dailyVolumeUSD = aliumDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  aliumDayData.dailyVolumeBNB = aliumDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  aliumDayData.dailyVolumeUntracked = aliumDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  aliumDayData.save();
